...
import { Edit2, Save, X, Upload, Calendar, Zap, Users, Trophy, Target, Clock, TrendingUp, Star, Award, Lock, ChevronRight, Flame, BookOpen, MessageSquare, Loader2 } from 'lucide-react'
...
export default function Profile() {
  ...
  async function fetchInitialData() {
    ...
    try {
      ...
      // Check if there are more proofs
      setHasMore(proofsSnapshot.docs.length === INITIAL_LOAD)

      // Save last document for pagination
      if (proofsSnapshot.docs.length > 0) {
        setLastDoc(proofsSnapshot.docs[proofsSnapshot.docs.length - 1])
      }
      ...
      // Build heatmap data from all available activity
      // For accurate heatmap, we need full year data
      await fetchHeatmapData()

      // Calculate stats
      const statsData = {
        totalProofs: currentUser.totalProofs || proofs.length,
      ...
      // Build activity timeline from recent proofs
      buildTimeline(proofs, userAchievements)

    } catch (error) {
      trackError(error, { action: 'fetchInitialData', userId: currentUser?.uid }, 'error', ErrorCategory.FIRESTORE)
  ...
  // Load more proofs (pagination)
  const loadMoreProofs = useCallback(async () => {
    if (!currentUser || !hasMore || loadingMore || !lastDoc) return
    ...
    try {
      ...
      const snapshot = await getDocs(moreProofsQuery)

      // Check if there are more
      setHasMore(snapshot.docs.length === PROOFS_PER_PAGE)
      ...
      toast.error('Failed to load more proofs')
    } finally {
      setLoadingMore(false)
    }
  }, [currentUser, hasMore, loadingMore, lastDoc])
  ...
  }, [hasMore, loadingMore, loadMoreProofs])

  // Build activity timeline
  function buildTimeline(proofs, userAchievements) {
    const timeline = proofs.slice(0, 10).map(post => ({
      id: post.id,
      type: 'proof',
  ...
  function getAchievementTypeColor(type) {
    const colors = {
      streak: 'bg-glow-500/20 text-glow-400 border-glow-500/30',
      proofs: 'bg-brand-500/20 text-brand-400 border-brand-500/30',
      pods: 'bg-purple-500/20 text-purple-400 border-purple-500/30',
      partners: 'bg-green-500/20 text-green-400 border-green-500/30',
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-12">
        ...
        <div className="flex flex-col md:flex-row items-start gap-6">
          ...
          <div className="relative">
            <div {...getRootProps()} className={`cursor-pointer ${editMode ? 'ring-2 ring-brand-400' : ''} rounded-full`}>
              <input {...getInputProps()} />
              <Avatar user={currentUser} size="xl" className="w-32 h-32" />
              {editMode && (
            ...
            {editMode ? (
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full bg-white/5 border ${errors.displayName ? 'border-red-500' : 'border-white/10'} rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-brand-400`}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className={`w-full bg-white/5 border ${errors.bio ? 'border-red-500' : 'border-white/10'} rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-brand-400 min-h-[100px]`}
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full bg-white/5 border ${errors.goals ? 'border-red-500' : 'border-white/10'} rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-brand-400`}
                ...
                <div className="flex gap-2">
                  <button onClick={handleSave} disabled={saving} className="btn-primary px-6 py-2 disabled:opacity-50 flex items-center gap-2">
                    {saving ? <><Loader2 className="h-4 w-4 animate-spin" />Saving...</> : <><Save size={16} />Save</>}
                  </button>
                  <button onClick={handleCancel} disabled={saving} className="px-6 py-2 border border-white/10 rounded-lg hover:bg-white/5 transition disabled:opacity-50 flex items-center gap-2">
                    <X size={16} />Cancel
                  </button>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={() => setEditMode(true)} className="btn-primary px-4 py-2 flex items-center gap-2">
                    <Edit2 size={16} />Edit Profile
                  </button>
                </div>

                {currentUser.bio && <p className="text-zinc-300 mb-4">{currentUser.bio}</p>}

                {currentUser.goals && currentUser.goals.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-4">
                    {currentUser.goals.map((goal, idx) => (
                      <span key={idx} className="px-3 py-1 bg-brand-400/20 text-brand-400 rounded-full text-sm">{goal}</span>
                    ))}
                  </div>
                )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-6 pt-6 border-t border-white/10">
          <div className="text-center p-4 bg-white/5 rounded-xl">
            <div className="flex items-center justify-center gap-2 text-2xl font-bold mb-1">
              <BookOpen className="text-brand-400" size={20} />{stats.totalProofs}
            </div>
            <div className="text-sm text-zinc-400">Total Proofs</div>
          </div>
          <div className="text-center p-4 bg-white/5 rounded-xl">
            <div className="flex items-center justify-center gap-2 text-2xl font-bold mb-1">
              <Flame className="text-glow-500" size={20} />{stats.streak}
            </div>
            <div className="text-sm text-zinc-400">Current Streak</div>
          </div>
          <div className="text-center p-4 bg-white/5 rounded-xl">
            <div className="flex items-center justify-center gap-2 text-2xl font-bold mb-1">
              <TrendingUp className="text-green-400" size={20} />{stats.longestStreak}
            </div>
            <div className="text-sm text-zinc-400">Best Streak</div>
          </div>
          <div className="text-center p-4 bg-white/5 rounded-xl">
            <div className="flex items-center justify-center gap-2 text-2xl font-bold mb-1">
              <Users className="text-purple-400" size={20} />{stats.podsJoined}
            </div>
            <div className="text-sm text-zinc-400">Pods Joined</div>
          </div>
          <div className="text-center p-4 bg-white/5 rounded-xl">
            <div className="flex items-center justify-center gap-2 text-2xl font-bold mb-1">
              <Trophy className="text-yellow-400" size={20} />{unlockedCount}/{totalAchievements}
            </div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-8">
        <GitHubHeatmap activityData={heatmapData} currentStreak={stats.streak} longestStreak={stats.longestStreak} />
      </motion.div>
        ...
        {[
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition whitespace-nowrap ${
              activeTab === tab.id ? 'bg-brand-500 text-white' : 'bg-white/5 text-zinc-400 hover:bg-white/10'
            }`}
          >
      ...
      <AnimatePresence mode="wait">
            ...
            <div className="glass p-6 rounded-2xl">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-semibold flex items-center gap-2">
                  <Trophy className="text-yellow-400" size={20} />Recent Badges
                </h2>
                <button onClick={() => setActiveTab('achievements')} className="text-brand-400 hover:text-brand-300 text-sm flex items-center gap-1">
              ...
              <div className="mt-4 pt-4 border-t border-white/10">
                ...
                <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                  <motion.div initial={{ width: 0 }} animate={{ width: `${completionPercentage}%` }} transition={{ duration: 1, delay: 0.3 }} className="h-full bg-gradient-to-r from-glow-500 to-brand-500" />
                </div>
              </div>
            </div>

            {/* Recent Proofs with Pagination */}
            <div className="glass p-6 rounded-2xl">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-semibold flex items-center gap-2">
                  <BookOpen className="text-brand-400" size={20} />Recent Proofs
                </h2>
                <Link to="/analytics" className="text-brand-400 hover:text-brand-300 text-sm flex items-center gap-1">
              ...
              {recentProofs.length === 0 ? (
                ...
                <div className="space-y-3 max-h-[300px] overflow-y-auto">
                  {recentProofs.slice(0, 5).map((proof, idx) => (
                    <div key={proof.id} className="p-3 bg-white/5 rounded-lg border border-white/10 hover:border-brand-500/30 transition">
                  ...
                  {hasMore && (
                    <div ref={loadMoreRef} className="flex justify-center py-4">
                      {loadingMore && <Loader2 className="h-5 w-5 text-brand-400 animate-spin" />}
                    </div>
                  )}
                </div>
              )}
        ...
        {/* Achievements Tab and Activity Tab remain similar to original */}
        {activeTab === 'achievements' && (
          <motion.div key="achievements" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="glass p-6 rounded-2xl">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h2 className="text-xl font-semibold flex items-center gap-2">
                  <Trophy className="text-yellow-400" size={20} />All Achievements
                </h2>
                <p className="text-sm text-zinc-400 mt-1">
                  {unlockedCount} of {totalAchievements} unlocked ({completionPercentage}%)
                </p>
              </div>
              <button onClick={() => setShowAllAchievements(!showAllAchievements)} className="text-brand-400 hover:text-brand-300 text-sm">
            ...
            {['streak', 'proofs', 'pods', 'partners', 'verified', 'special'].map(type => {
              ...
              const visibleAchievements = showAllAchievements ? typeAchievements : typeAchievements.filter(a => a.unlocked)
              ...
              return (
                <div key={type} className="mb-6 last:mb-0">
                  <h3 className="text-sm font-medium text-zinc-400 mb-3 uppercase tracking-wider">{typeLabels[type]}</h3>
                          ...
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                ...
                                {achievement.unlocked && (
                                  <span className="text-xs px-1.5 py-0.5 bg-green-500/20 text-green-400 rounded">Unlocked</span>
                                )}
                              </div>
                              ...
                              {!achievement.unlocked && (
                                <div className="mt-2">
                                  ...
                                  <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
                                    <div className="h-full bg-brand-500/50" style={{ width: `${progress.percentage}%` }} />
                                  </div>
                                </div>
                              )}
                            </div>
        ...
        {activeTab === 'activity' && (
          <motion.div key="activity" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="glass p-6 rounded-2xl">
            <h2 className="text-xl font-semibold flex items-center gap-2 mb-6">
              <Clock className="text-brand-400" size={20} />Activity Timeline
            </h2>
                  ...
                  {activityTimeline.map((item, idx) => (
                    <div key={item.id} className="flex items-start gap-4 relative">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 z-10 ${
                        item.type === 'proof' ? 'bg-brand-500/20 text-brand-400' :
                        item.type === 'achievement' ? 'bg-yellow-500/20 text-yellow-400' :
                        'bg-white/10 text-zinc-400'
                      }`}>
                        {item.type === 'proof' ? <BookOpen size={18} /> :
                         item.type === 'achievement' ? <span className="text-lg">{item.icon}</span> :
                        ...
                        <div className="p-4 bg-white/5 rounded-xl border border-white/10 hover:border-brand-500/30 transition">
                          {item.type === 'proof' ? (
                            ...
                            <p className="text-zinc-200 text-sm flex items-center gap-2">
                              <Trophy size={14} className="text-yellow-400" />{item.content}
                            </p>
                          )}
                          <p className="text-xs text-zinc-500 mt-2">{formatRelativeTime(item.date)}</p>
                        </div>
                      </div>
                    </div>
                  ))}
      ...
      {/* Quick Stats Footer */}
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="mt-6 glass p-4 rounded-xl flex flex-wrap items-center justify-center gap-6 text-sm">
        ...
        <Link to="/analytics" className="flex items-center gap-2 text-brand-400 hover:text-brand-300 transition">
          <Target size={16} />
