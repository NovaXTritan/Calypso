...
export const SYSTEM_PROMPT_CHAT = `You are Cosmos Journal Analyst — a thoughtful, evidence-aware mentor embedded in a peer accountability platform for ambitious Indian students.
...
export const SYSTEM_PROMPT_ANALYSIS = `You are generating a structured weekly journal analysis for a student on the Cosmos accountability platform.
...
export function buildChatContext(
  ...
  if (goals) {
    ...
    parts.push(
      `Primary: ${goals.primaryGoal} (Target: ${goals.targetDate.toDate().toLocaleDateString("en-IN", { year: "numeric", month: "short", day: "numeric" })})`
    );
    parts.push("Focus Areas:");
    for (const fa of goals.focusAreas) {
      parts.push(
        `- ${fa.area}: ${fa.targetHoursPerWeek} hrs/week (Priority ${fa.priority})`
      );
    }
    parts.push("");
    ...
    parts.push(
      `Health: ${lb.healthImportance}/10 | Family: ${lb.familyImportance}/10 | Friends: ${lb.friendsImportance}/10 | Exploration: ${lb.explorationImportance}/10 | Rest: ${lb.restImportance}/10`
    );
    parts.push("");
  }
  ...
  if (previousInsight) {
    ...
    if (previousInsight.weeklyExperiment) {
      parts.push(
        `Experiment: "${previousInsight.weeklyExperiment.title}" — did they follow through?`
      );
    }
